...
final class LayoutProxy {
	...
	//private final Map<LuaValue, Field> fieldsByProxy = new WeakHashMap<LuaValue, Field>();
	//private final Map<LuaValue, Track> tracksByProxy = new WeakHashMap<LuaValue, Track>();

	LayoutProxy(Layout layout) {
		this.layout = layout;
	}

	public LuaValue layout() {
	...
	private LuaValue field(final Field field) {
		...
		res.set("awaitState", new TwoArgFunction() {
			...
			public LuaValue call(LuaValue arg1, LuaValue arg2) {
				...
				Runnable runnable = runnableFromLuaFunction(arg2);
	...
	private LuaValue track(final Track track) {
		...
		res.set("setActive", new OneArgFunction() {
			@Override
			public LuaValue call(LuaValue arg) {
				track.activate(runnableFromLuaFunction(arg));

				return NIL;
			}
		});

		//tracksByProxy.put(res, track);
	...
	private static Runnable runnableFromLuaFunction(final LuaValue arg) {
		Runnable runnable;

		if (arg.isnil()) {
			runnable = RunnableUtil.emptyRunnable;
